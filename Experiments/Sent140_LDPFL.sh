...
for seed_val in "${seeds[@]}"; do
    ...
    for epsilon in "${epsilons[@]}"; do
        for lambda in "${lambda_params[@]}"; do
            cmd="python main.py --dataset Sent140 --iid --method LDPFL --device GPU --num_clients 50 --num_rounds 100 --epsilon $epsilon --lambda_param $lambda --seed $seed_val --lr 0.001"
            run_with_retry "$cmd"
        done
    done
done