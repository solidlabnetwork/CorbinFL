...
for seed_val in "${seeds[@]}"; do
    ...
    for lambda in "${lambda_params[@]}"; do
        cmd="python main.py --dataset FEMNIST --method LaplaceLDP --device GPU --num_clients 350 --num_rounds 200 --epsilon $epsilon --lambda_param $lambda --seed $seed_val --lr 0.0003"
        run_with_retry "$cmd"
    done
done